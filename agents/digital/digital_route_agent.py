...
docker run --rm \\
  ...
  -e PDK_ROOT=/pdk \\
  {image} \\
  bash -lc 'set -e; cd /work && openlane --flow Classic --run-tag {run_tag} --to OpenROAD.DetailedRouting route/config.json'
"""
    ...
    _write(os.path.join(logs_dir,"openlane_route.log"), out)
    ...
    summary={"workflow_id":workflow_id,"agent":AGENT_NAME,"status":"ok" if rc==0 else "failed","return_code":rc,
             "outputs":{"metrics_json":"digital/route/metrics.json" if metrics else None,
                        "primary_def":"digital/route/primary.def" if primary_def else None,
                        "log":"digital/route/logs/openlane_route.log","openlane_run_dir":latest}}
    _write(os.path.join(stage_dir,"route_summary.json"), json.dumps(summary, indent=2))
    _write(os.path.join(stage_dir,"route_summary.md"), f"# Route\n\n- status: `{summary['status']}` (rc={rc})\n")

    try:
        save_text_artifact_and_record(workflow_id,AGENT_NAME,"digital","route/config.json", json.dumps(cfg, indent=2))
        save_text_artifact_and_record(workflow_id,AGENT_NAME,"digital","route/constraints/top.sdc", sdc_text)
    ...
    state.setdefault("digital",{})["route"]={"status":summary["status"],"stage_dir":stage_dir,"metrics_json":metrics,"primary_def":primary_def}
    return state