...
def run_agent(state: dict) -> dict:
    ...
    # Base config selection (foundry/openlane preferred)
    ...
    if not stage_netlists:
        raise RuntimeError("STA preplace: missing run_work/inputs/netlist/*.v (synth should populate it).")
...
"""
    ...
    _write(os.path.join(logs_dir, "openlane_sta_preplace.log"), out)
    ...
    summary = {
        ...
        "return_code": rc,
        "outputs": {
            "metrics_json": "digital/sta_preplace/metrics.json" if metrics else None,
            "log": "digital/sta_preplace/logs/openlane_sta_preplace.log",
            "openlane_run_dir": latest,
        },
    }
    _write(os.path.join(stage_dir, "sta_preplace_summary.json"), json.dumps(summary, indent=2))
    _write(os.path.join(stage_dir, "sta_preplace_summary.md"),
