...
def _latest_run_dir(run_work_dir: str) -> str | None:
    run_roots = [
        ...
        os.path.join(run_work_dir, "fill", "runs"),
    ]
    dirs = []
...
def _resolve_sdc_from_state(state: dict, workflow_dir: str) -> str | None:
    ...
    for stage in ["route", "sta_postroute", "sta_postcts", "cts", "place", "floorplan", "impl_setup", "synth"]:
...
def _resolve_config_from_state(state: dict, workflow_dir: str) -> str | None:
    ...
    cand = route_state.get("openlane_config")
    if cand and os.path.exists(cand):
        logger.info(f"{AGENT_NAME}: selected config from state.digital.route -> {cand}")
        return cand
...
docker run --rm \\
  ...
  -e PDK_ROOT=/pdk \\
  {openlane_image} \\
  bash -lc 'set -e; cd /work && openlane --flow Classic --run-tag {run_tag} --override-config RUN_LINTER=False --to OpenROAD.FillInsertion fill/config.json'
"""
    ...
    _write_text(os.path.join(stage_dir, "fill_summary.md"), f"# Fill\n\n- status: `{summary['status']}` (rc={rc})\n")
    ...
    state.setdefault("digital", {})["fill"] = {
        ...
        "input_resolution_log": os.path.join(logs_dir, "fill_input_resolution.log"),
        "openlane_run_dir": latest,
    }
    return state
