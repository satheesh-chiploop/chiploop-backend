...
def _record_text(workflow_id: str, agent_name: str, subdir: str, filename: str, content: str) -> Optional[str]:
    try:
        ...
        return None
def _gen_cocotb_test(spec: Dict[str, Any], top: str, clocks: List[str], resets: List[Dict[str, Any]], soc_mode: bool = False) -> str:
    ...
    if clk:
        clk_seq = (
            f"    if hasattr(dut, '{clk}'):\n"
            f"        cocotb.start_soon(Clock(dut.{clk}, 10, units='ns').start())\n"
        )
    else:
...
- Uses Digital Spec JSON to detect ports/clocks/resets when available
- In System_SIM SoC mode, avoids assuming all digital-spec ports exist on the assembled top
...
async def smoke_test(dut):
    """Basic sanity: optional clock/reset + one simple stimulus window."""
    seed = _seed()
    random.seed(seed)
...
{rst_seq.rstrip()}
    ...
    # Initialize DUT inputs (except clk/rst) when discoverable
{init_block}
    ...
    if {bool(clk)} and hasattr(dut, {json.dumps(clk) if clk else "None"}):
        for _ in range(5):
            await RisingEdge(getattr(dut, {json.dumps(clk)}))
    else:
...
def run_agent(state: dict) -> dict:
    ...
    soc_mode = bool(state.get("soc_top_sim_module"))
...
$(error cocotb-config not found on PATH. Please install/activate cocotb in this runtime before running make)
endif
