...
def _gen_makefile(top: str) -> str:
    ...
    venv_cocotb_config = os.path.join(py_dir, "cocotb-config")
    cocotb_config_bin = (
        venv_cocotb_config
        if os.path.exists(venv_cocotb_config)
        else (shutil.which("cocotb-config") or "cocotb-config")
    )
...
COCOTB_CONFIG ?= {cocotb_config_bin}
SIM           ?= verilator
...
$(error cocotb-config --makefiles failed. Please ensure cocotb is installed in the active backend venv and COCOTB_CONFIG points to the correct binary)
endif
