...
def _manifest(contract: Dict[str, Any], api_contract: Dict[str, Any], adapter_crate: str, adapter_package_name: str) -> Dict[str, Any]:
...
def _markdown_summary(manifest: Dict[str, Any], files: List[str]) -> str:
    lines = [
        ...
        f"- Adapter crate: `{manifest.get('adapter_crate') or 'unavailable'}`",
        f"- Adapter package: `{manifest.get('adapter_package_name') or 'unavailable'}`",
        "",
        "## Adapter contract",
        "",
    ]
...
def run_agent(state: dict) -> dict:
    ...
    files_to_write = {
        f"{adapter_crate}/src/adapter/register_adapter.rs": _render_register_adapter(),
        f"{adapter_crate}/src/adapter/device_adapter.rs": _render_device_adapter(
            bool(fc.get("interrupt_mapping_path")),
            bool(fc.get("dma_integration_path")),
            bool(fc.get("power_mode_path")),
        ),
        f"{adapter_crate}/src/error.rs": _render_error_module(),
    }
...
''', subdir=f"{OUTPUT_SUBDIR}/{adapter_crate}/src/adapter")
