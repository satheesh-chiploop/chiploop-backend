...
def _write_magic_import_tcl(
    ...
    lines = [
        ...
        "set chiploop_cells {}",
        "catch {set chiploop_cells [cellname list allcells]}",
        "if {[llength $chiploop_cells] == 0} {catch {set chiploop_cells [cellname list]}}",
        f"foreach chiploop_cell $chiploop_cells {{if {{$chiploop_cell ne \"{module_name}\"}} {{catch {{cellname delete $chiploop_cell}}}}}}",
        "gds flatten true",
        "catch {gds flatglob *}",
